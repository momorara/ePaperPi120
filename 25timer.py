...
def draw_frame(draw, x0, y0, width, height, num_frames):
    ...
    # 外枠を描画します。
    draw.rectangle((x0, y0, x0 + width +2, y0 + height - 2), outline="black", fill="white", width=1)
    # 各内枠を描画します。
    for i in range(num_frames):
        ...
        if i != 4:
            ...
            draw.rectangle((x, y0 + 2, x + d_width-2, y0 + height - 4), outline="black", fill="white", width=1)
...
def fill_frame(draw, x0, y0, d_width, height, n):
    ...
    """
    # 塗りつぶす枠の左上の座標を計算します。
    xn = (x0 + 2) + d_width * n + n * 2 +2
    yn = y0 + 2
    # 徐々に小さくなる矩形で枠を塗りつぶします。
    for i in range(d_width // 4 -2):
        width_n  = d_width - i * 2 -6
        height_n = height - 4 - i * 2 -0  # '4' を引くのは上下のパディングを考慮するためです。
...
def main():
    ...
    # タイマーのタイトルテキストを描画
    draw.text((0, 0), "25-minute timer", font=ep_lib.font_set("gos", 22), fill=0)
    x0, y0 = 0, 25
